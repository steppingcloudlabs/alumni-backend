                        ...
                        if (findticket) {
                            if (findticket.esclation_manager == 'esclation_manager_2') {
                                ...
                                const savenotification = new Notification({
                                    user: id,
                                    content: 'Notification trigerred, Please look into the matter posted at your dashboard',
                                });
        ...
        return new Promise(async(resolve, reject) => {
            try {
                ...
                if (Date.now() > expirytimefromtoken) {
                    ...
                    const result = await User.find({ userType: 'admin' }, { email: 1, userType: 1, userid: 1, companyname: 1 });
                    resolve(result);
                }
            } catch (error) {
                reject(error);
        ...
        return new Promise(async(resolve, reject) => {
            try {
                ...
                if (Date.now() > expirytimefromtoken) {
                    ...
                    const result = await Manager.find({ type: esclation_level_manager_1 }, { email: 1, userType: 1, userid: 1, companyname: 1 });
                    resolve(result);
                }
            } catch (error) {
                reject(error);
